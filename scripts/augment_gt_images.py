...
DEFAULT_INPUT_DIR = DEFAULT_PROJECT_ROOT / "seals-dataset/ground_truth/01_510_800_820_280_114"
DEFAULT_OUTPUT_DIR = DEFAULT_PROJECT_ROOT / "seals-dataset/ground_truth/01_510_800_820_280_114_augmented"
...
def augment_image(image: np.ndarray, rng: np.random.Generator, args: argparse.Namespace) -> np.ndarray:
    ...
    augmented = add_gradient_brightness(
        augmented,
        rng,
        (args.min_gradient_strength, args.max_gradient_strength),
    )
    augmented = add_random_rectangles(
        augmented,
        rng,
        (args.min_rectangles, args.max_rectangles),
        (args.min_rectangle_brightness, args.max_rectangle_brightness),
    )
    ...
    augmented = add_salt_and_pepper_noise(
        augmented,
        rng,
        (args.min_noise_amount, args.max_noise_amount),
        args.salt_ratio,
    )
    return augmented
...
def output_path_for(
    ...
    relative_parent = image_path.relative_to(input_dir).parent if recursive else Path()
    filename = f"{image_path.stem}_aug{variant_index:03d}{image_path.suffix}"
    return output_dir / relative_parent / filename


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create augmented copies of ground-truth seal images."
    )
    ...
    parser.add_argument("--min-noise-amount", type=float, default=0.002)
    parser.add_argument("--max-noise-amount", type=float, default=0.015)
    parser.add_argument("--salt-ratio", type=float, default=0.5)
    parser.add_argument("--blur-probability", type=float, default=0.5)
    parser.add_argument("--max-blur-kernel-size", type=int, default=5)
    parser.add_argument("--min-rectangles", type=int, default=1)
    parser.add_argument("--max-rectangles", type=int, default=4)
    parser.add_argument("--min-rectangle-brightness", type=int, default=-80)
    parser.add_argument("--max-rectangle-brightness", type=int, default=80)
    parser.add_argument("--min-gradient-strength", type=int, default=-60)
    parser.add_argument("--max-gradient-strength", type=int, default=60)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.min_scale <= 0 or args.max_scale <= 0 or args.min_scale > args.max_scale:
        raise ValueError("Scale bounds must be positive and --min-scale <= --max-scale")
    if not 0 <= args.min_noise_amount <= args.max_noise_amount <= 1:
        raise ValueError("Noise amount bounds must satisfy 0 <= min <= max <= 1")
    if not 0 <= args.salt_ratio <= 1:
        raise ValueError("--salt-ratio must be between 0 and 1")
    if not 0 <= args.blur_probability <= 1:
        raise ValueError("--blur-probability must be between 0 and 1")
    if args.max_blur_kernel_size % 2 == 0:
        raise ValueError("--max-blur-kernel-size must be odd")
    if args.min_rectangles < 0 or args.min_rectangles > args.max_rectangles:
        raise ValueError("Rectangle bounds must satisfy 0 <= min <= max")
    if args.min_rectangle_brightness > args.max_rectangle_brightness:
        raise ValueError("Rectangle brightness bounds must satisfy min <= max")
    if args.min_gradient_strength > args.max_gradient_strength:
        raise ValueError("Gradient strength bounds must satisfy min <= max")
...
def main() -> None:
    ...
    image_paths = collect_images(args.input_dir, args.recursive)
    if not image_paths:
    ...
    for image_path in image_paths:
        ...
        for variant_index in range(1, args.variants_per_image + 1):
            ...
            augmented = augment_image(image, rng, args)
    ...
    print(f"Variants per image: {args.variants_per_image}")
